    ...
    class Platform
    {
        ...
        public Platform(Size s, Vector2 p)
        {
            ...
            if (size == Size.small)
            {
                ...
                collisionRect = new Rectangle((int)pos.X, (int)pos.Y, 16, 1);
            }
            if (size == Size.medium)
            {
                bounds = new Rectangle((int)pos.X, (int)pos.Y, 128, 32);
                collisionRect = new Rectangle((int)pos.X, (int)pos.Y, 32, 1);
            }
            if (size == Size.large)
            {
                bounds = new Rectangle((int)pos.X, (int)pos.Y, 256, 64);
                collisionRect = new Rectangle((int)pos.X, (int)pos.Y, 64, 1);
            }
